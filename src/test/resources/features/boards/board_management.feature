Feature: Trello Board Management
 ...
 Scenario Outline: Templates (UI)
	...
	Examples:
		...
		|myKanban 	|  Kanban Template			| Backlog, Design,To Do, Doing, Code Review, Testing, Done |
		|myBikes 	|  Bike Repair Pipeline 	| Needs Parts ordered, At Paint, Ready to Build, Wheel Builds, Services/Repairs |
		|myRoadmap 	|  Product Roadmap 			| Ideas, Research and Design, Estimating, Sprint Candidates |
		|myRetro 	|  Sprint Retrospectives	| Sprint Retrospectives Template:, Went Well, Needs To Change, Questions & Discussion,  Action Items |


Scenario: Perform various trello board actions and see the actions are correctly returned in the actions request (REST)
	Given I create a new board called 'myBoard'
	...
	And I add checklist 'myChecklist' with item 'check1' to the card
	And I check the checklist item
	And I remove the checklist from the card
	And I add attachment url 'http://www.google.com' with name 'myLink' to the card
	...
	Then the total number of actions should be 10
 	And the actions should include the following details:
 		...
 		|createBoard   				| data.board.name			|myBoard 					|
 		|createList   				| data.list.name			|ToDo 						|
 		|createCard   		 		| data.card.name    		|Do something useful    	|
 		|commentCard   				| data.text    				|is this useful enough?		|
		|addChecklistToCard   		| data.checklist.name		|myChecklist				|
		|updateCheckItemStateOnCard | data.checkItem.state		|complete					|
		|removeChecklistFromCard 	| data.checklist.name		|myChecklist				|
		|addAttachmentToCard 		| data.attachment.name		|myLink						|
		|deleteAttachmentFromCard	| data.attachment.name		|myLink						|
